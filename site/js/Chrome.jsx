...
const { SearchBar, Button } = window.GEPCOFormaciNDesignSystem_bb6094;
...
function useCart() {
  const [items, setItems] = React.useState(window.GEPCO_CART ? window.GEPCO_CART.items() : []);
  React.useEffect(() => {
    if (!window.GEPCO_CART) return;
    window.GEPCO_CART.on(setItems);
    setItems(window.GEPCO_CART.items());
  }, []);
  return items;
}
...
function CartDrawer({ open, onClose, onNav }) {
  ...
  const set = (id, q) => window.GEPCO_CART.setQty(id, q);
      ...
      <aside role="dialog" aria-modal="true" aria-label="Carrito de formaciones"
            ...
            <div key={it.id} style={{ display: "flex", gap: "var(--space-4)", background: "var(--surface-card)", border: "1px solid var(--border-default)", borderRadius: "var(--radius-md)", padding: "var(--space-4)" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px" }}>
                  ...
                  <button type="button" onClick={() => window.GEPCO_CART.remove(it.id)} style={{ border: "none", background: "transparent", cursor: "pointer", fontSize: "var(--text-sm)", color: "var(--text-subtle)", textDecoration: "underline" }}>Quitar</button>
                </div>
              </div>
            </div>
          ))}
        ...
        {items.length > 0 && (
          <footer style={{ padding: "var(--space-6)", borderTop: "1px solid var(--border-default)", background: "var(--surface-card)", display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
            ...
            <button type="button" onClick={() => window.GEPCO_CART.clear()} style={{ border: "none", background: "transparent", cursor: "pointer", fontSize: "var(--text-sm)", color: "var(--text-subtle)", textDecoration: "underline", alignSelf: "center" }}>Vaciar carrito</button>
          </footer>
        )}
      </aside>
    </div>
...
function Footer() {
  const nav = (typeof window !== "undefined" && window.onNav) || function () {};
  const offices = [
